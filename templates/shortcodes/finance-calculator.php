<?php
/**
 * Generic Finance Calculator Template
 *
 * Renders the full, user-insertable [dealerschoice_finance_calculator]
 * shortcode: amount/rate/term/down-payment inputs, a quick loan-summary
 * result, and a collapsed full amortization schedule.
 *
 * ── THEME OVERRIDE ───────────────────────────────────────────────────────────
 * Copy this file to {your-theme}/dealerschoice/shortcodes/finance-calculator.php
 * to customise the layout without editing the plugin.
 *
 * ── AVAILABLE VARIABLES ──────────────────────────────────────────────────────
 * $title                 (string) Heading above the form.
 * $default_amount        (float|string) Pre-filled amount financed, or '' if blank.
...
/** @var string $title */
/** @var float|string $default_amount */
...
?>
<div
    class="dc-finance-calc"
    id="<?php echo esc_attr( $instance_id ); ?>"
    data-calc-variant="full"
    data-default-down-payment-percent="<?php echo esc_attr( $down_payment_pct ); ?>"
>
    <?php if ( $title ) : ?>
        <h2 class="dc-finance-calc-title"><?php echo esc_html( $title ); ?></h2>
    <?php endif; ?>
    ...
    <form class="dc-finance-calc-form" method="get" novalidate>
        <div class="dc-finance-calc-fields">
            ...
            <div class="dc-field">
                <label for="<?php echo esc_attr( $instance_id ); ?>-amount">
                    <?php esc_html_e( 'Amount Financed', 'dealerschoice' ); ?>
                ...
                <input
                    ...
                    aria-describedby="<?php echo esc_attr( $instance_id ); ?>-amount-error"
                >
                <p id="<?php echo esc_attr( $instance_id ); ?>-amount-error" class="dc-field-error" role="alert" hidden></p>
            </div>
    ...
    <details class="dc-finance-schedule-details" hidden>
        ...
        <div class="dc-finance-schedule-table-wrap"></div>
    </details>
